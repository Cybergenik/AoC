...
class Computer:
    ...
    def run_inst(self, inst, operand, inst_ptr):
        if inst == 0:
            #print(f"RB =  RA//2^{self.get_operand_val(operand)}")
            self.RA = self.RA//2**self.get_operand_val(operand)
        elif inst == 1:
            #print(f"RB =  RB ^ {operand}")
            self.RB ^= operand
        elif inst == 2:
            #print(f"RB = {self.get_operand_val(operand)}%8")
            ...
            #print(f"print {self.get_operand_val(operand)}%8")
