...
def main() -> int:
    ...
    logs.sort()
    guards = defaultdict(lambda: Counter())
    curr = None
    s = None
    e = None
    for log in logs:
        if log.guard:
            ...
            e = log
            guards[curr].update(range(s.min, e.min))
    s_guard = max(guards.items(), key= lambda x: x[1].total())
    top_guard = s_guard[0]
    top_min = s_guard[1].most_common(1)[0][0]
